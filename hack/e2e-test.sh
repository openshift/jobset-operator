...
function deploy_jobset_operator {
  echo "Define operator and operand images built in CI"
  sed -i -e "s|\${OPERAND_IMAGE}|$RELATED_IMAGE_OPERAND_IMAGE|g" -e "s|\${OPERATOR_IMAGE}|$OPERATOR_IMAGE|g" deploy/12_deployment.yaml
  ...
  oc wait deployment jobset-operator -n openshift-jobset-operator --for=create --timeout=2m
  oc wait deployment jobset-operator -n openshift-jobset-operator --for=condition=Available --timeout=5m
  oc wait deployment jobset-controller-manager -n openshift-jobset-operator --for=create --timeout=2m
...
function run_e2e_operand_tests() {
  ...
  git clone -b "${BRANCH}" "https://github.com/openshift/kubernetes-sigs-jobset" "${OPERAND_CLONE_PATH}"
  pushd "${OPERAND_CLONE_PATH}"
