...
func newOperatorTestCommand(ctx context.Context) (*cobra.Command, error) {
	...
	cmd := &cobra.Command{
		Use:   "jobset-operator-tests-ext",
		Short: "A binary used to run jobset-operator tests as part of OTE.",
...
func prepareOperatorTestsRegistry() (*oteextension.Registry, error) {
	...
	extension := oteextension.NewExtension("openshift", "payload", "jobset-operator")
	...
	extension.AddSuite(oteextension.Suite{
		Name:        "openshift/jobset-operator/operator/serial",
		Parallelism: 1,
		Qualifiers: []string{
